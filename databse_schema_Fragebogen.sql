...
CREATE TABLE IF NOT EXISTS fb_fragebogen (
    ...
    -- Zeiterfassung (time tracking) - enabled by default
...
-- ============================================================================
-- 10. ZEITERFASSUNG SUBMISSIONS TABLE (fb_zeiterfassung_submissions)
-- Time tracking data per GL per market visit
-- ============================================================================
CREATE TABLE IF NOT EXISTS fb_zeiterfassung_submissions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    
    response_id UUID REFERENCES fb_responses(id) ON DELETE CASCADE,
    fragebogen_id UUID REFERENCES fb_fragebogen(id) ON DELETE CASCADE,
    gebietsleiter_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    market_id VARCHAR(50) NOT NULL REFERENCES markets(id) ON DELETE CASCADE,
    ...
    fahrzeit_von TIME,
    fahrzeit_bis TIME,
    fahrzeit_diff INTERVAL, -- stored as interval, displayed as HH:MM
    ...
    -- Food percentage (0-100), rest is Pets
    food_prozent INTEGER CHECK (food_prozent >= 0 AND food_prozent <= 100),
    
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    
    -- One zeiterfassung per response (when response exists)
    CONSTRAINT unique_zeiterfassung_per_response UNIQUE (response_id)
);

-- Indexes for fb_zeiterfassung_submissions
...
-- GLs can manage their own zeiterfassung submissions
